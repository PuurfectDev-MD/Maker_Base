<script lang="ts">
	...
	import '../../style.css';
	import { createEditor } from '../../editorConfig.js';
	...
	import { handleUpdate } from '../../../posts/posts.remote.js';
	...
	let selected = $state<string[]>(data.post.tag_ids ?? []);
	let description = $state(data.post.description);
	let isPublic = $state(data.post.is_public);
	...
	async function submitPost() {
		if (validatePresence()) {
			...
			const { type, message } = await handleUpdate({
				id: postId,
				content,
				title,
				isPublic,
	...
	onMount(() => {
		...
		try {
			// we only parse it if its a string
			initialBlocks =
				typeof data.post.content === 'string' ? JSON.parse(data.post.content) : data.post.content;
		} catch (e) {
			console.error('Content parsing failed', e);
		}

		editor = createEditor(initialBlocks);
...
{#if posting}
	...
	>
		<p class="px-8 py-4 text-xl md:text-2xl">Sending it to the world again! ...</p>
	</div>
{/if}
